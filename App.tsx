...
const App: React.FC = () => {
  ...
  const calculateEstimation = () => {
    ...
    // Estimate runtime based on safe interval or manual delay
    ...
    try {
      while (currentStartTime < endTime) {
        ...
        while (isPausedRef.current) {
          ...
          if (!isCollecting) return; // Handle cancellation
        }
        ...
        // Remove duplicates if any (though Binance shouldn't have them in sequential requests)
  ...
  const downloadCSV = () => {
    ...
    link.setAttribute('download', `${config.symbol}_${config.timeframe}_dataset.csv`);
    link.click();
  };
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <section className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-slate-200 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                  value={config.symbol}
              ...
              <div className="pt-4 space-y-3">
                {!isCollecting ? (
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button 
                      onClick={togglePause}
                      className="py-2.5 bg-slate-700 hover:bg-slate-600 text-white font-semibold rounded-lg"
                    >
                      {isPaused ? 'Resume' : 'Pause'}
                    </button>
                    <button 
                      onClick={handleStop}
                      className="py-2.5 bg-red-600 hover:bg-red-500 text-white font-semibold rounded-lg"
                    >
                      Cancel
                    </button>
                  </div>
            ...
            <div className="mt-6 p-3 bg-slate-950/50 border border-slate-800 rounded-lg">
              ...
              {est.totalRequests > 100 && (
                <p className="mt-2 text-[10px] text-amber-500/80 leading-tight">
                  ⚠️ Range requires many requests. Ensure high rate limit or longer delays.
                </p>
              )}
            </div>
          </section>
        ...
        <div className="lg:col-span-2 space-y-6 flex flex-col h-full">
          ...
          <section className="bg-slate-900 border border-slate-800 p-6 rounded-xl shadow-xl">
             <div className="flex justify-between items-end mb-4">
                <div>
                  ...
                  <p className="text-sm text-slate-500">
                    {stats ? `${config.symbol} ${config.timeframe}` : 'No active session'}
                  </p>
                </div>
                {stats && (
             ...
             <div className="h-3 bg-slate-800 rounded-full overflow-hidden mb-6">
                <div 
                  ...
                  style={{ width: `${stats ? (stats.completedRequests / stats.totalRequests) * 100 : 0}%` }}
                />
             </div>
          ...
          <section className="bg-slate-900 border border-slate-800 flex-1 rounded-xl shadow-xl flex flex-col min-h-[400px]">
            ...
            <div className="p-2 overflow-y-auto flex-1 font-mono text-xs space-y-1 bg-slate-950/40 custom-scrollbar">
              {logs.length === 0 && (
      ...
      <footer className="mt-12 pt-8 border-t border-slate-800 flex flex-col md:flex-row justify-between items-center text-slate-600 text-[10px] uppercase tracking-widest gap-4">
        <p>© 2024 AI Research Data Platform</p>
        <div className="flex gap-6">
          <span>Binance API v3 Compliant</span>
          <span>OHLCV Standard Format</span>
          <span>Zero gaps Policy</span>
        </div>
      </footer>
    </div>
  );
